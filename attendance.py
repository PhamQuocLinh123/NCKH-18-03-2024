...
class Attendance:
    ...
    def __init__(self,root):
        ...
        self.root.title("Quản lý")#tiêu đề
        ...
        img3 = img3.resize((1350, 700), PIL.Image.BILINEAR)# chỉnh kích thước ảnh về 1350x700
        ...
        img_time = img_time.resize((27, 27), PIL.Image.BILINEAR)#resize ảnh về kích thước 27x27
        ...
        label_Update_att=Label(Left_frame,bg="#F0FFF0",fg="#483D8B",text="Cập Nhật ",font=("times new roman", 18, "bold"))
        ...
        #id điểm danh
        auttendanceID_label = Label(left_inside_frame, text="Mã Nhận Dạng :", font=("times new roman", 12, "bold"),
                                bg="white")
        ...
        #id học sinh
        roll_label = Label(left_inside_frame, text="Mã Người Dùng :", font=("times new roman", 12, "bold"),
                                    bg="white")
        ...
        #tên hs
        nameLabel = Label(left_inside_frame, text="Tên:", font=("times new roman", 12, "bold"),
                                    bg="white")
        ...
        classLabel = Label(left_inside_frame, text="Nghành:", font=("times new roman", 12, "bold"),
                                    bg="white")
        ...
        timeLabel = Label(left_inside_frame, text="Giờ :", font=("times new roman", 12, "bold"),
                                    bg="white")
        ...
        # thời gian ra
        timeoutLabel = Label(left_inside_frame, text="Ngày:", font=("times new roman", 12, "bold"),
                          bg="white")
        ...
        img_btn2 = img_btn2.resize((220, 35), PIL.Image.BILINEAR)
        ...
        img_btn1 = img_btn1.resize((150, 35), PIL.Image.BILINEAR)
        ...
        search_combo = ttk.Combobox(Right_frame, font=("times new roman", 11, "bold"),textvariable=self.var_com_search, state="read only",
                                    width=13)
        search_combo["values"] = ("ID  ", "Ngày", "ID Học sinh","ID Buổi học")
        ...
        img_btn3 = img_btn3.resize((110, 35), PIL.Image.BILINEAR)
        ...
        self.AttendanceReportTable.heading("class", text="Nghành")
    ...
    def fetch_data(self):
        mydata.clear()  # Clear data from the existing list
        ...
        my_cursor.execute(
            "SELECT IdAuttendance, Student_id, Name, Class, Time_in, Date FROM attendance")  # Select specific columns
        data = my_cursor.fetchall()

        if len(data) != 0:
            self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())  # Clear existing table data
            for i in data:
                self.AttendanceReportTable.insert("", END, values=i)  # Insert fetched data into the table
                mydata.append(i)
    ...
    def exportCsv(self):
        try:
            ...
            df = pd.DataFrame(mydata,
                              columns=['Mã ', 'ID Học sinh', 'Tên Học sinh', 'Lớp biên chế', 'Giờ vào','Ngày'])
    ...
    def openImage(self):#hàm mở ảnh đã điểm danh
        if self.var_atten_id == "":
            ...
            if not os.path.exists("DiemDanhImage\ " + self.var_atten_id.get() +"Ra" +".jpg"):#nếu ko có ảnh điểm danh ra:
                messagebox.showerror("Lỗi", "Không tìm thấy ảnh  ", parent=self.root)
