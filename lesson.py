...
class Lesson:
    def __init__(self,root):
        ...
        img3 = img3.resize((1350, 700), PIL.Image.BILINEAR)
        ...
        label_Update_att = Label(Left_frame, bg="#F0FFF0", fg="#483D8B", text="Thông tin ca làm việc",
        ...
        # lessonid
        auttendanceID_label = Label(left_inside_frame, text="Mã ca làm việc:", font=("times new roman", 11, "bold"),
                                    bg="white")
        ...
        self.var_timestart.set("00:00:00")  # Set default start time
        ...
        self.var_timeend.set("23:59:59")  # Set default end time
        ...
        classLabel = Label(left_inside_frame, text="Phòng:", font=("times new roman", 11, "bold"),
                           bg="white")
        ...
        search_combo = ttk.Combobox(Right_frame, font=("times new roman", 11, "bold"), textvariable=self.var_com_search,
                                    state="read only",
                                    width=13)
        search_combo["values"] = ("Mã ca làm việc", "Phòng","Ngày")
        ...
        self.AttendanceReportTable.heading("id", text="Mã ca làm việc")
        ...
        self.AttendanceReportTable.heading("class", text="Phòng")
    ...
    def timestart(self, event):
        if len(self.timestart_entry.get()) == 2:
            self.timestart_entry.insert(END, ":")
        elif len(self.timestart_entry.get()) == 5:
            self.timestart_entry.insert(END, ":")
        elif len(self.timestart_entry.get()) == 9:
            self.timestart_entry.delete(8, END)

    def timeend(self, event):
        if len(self.timeend_entry.get()) == 2:
            self.timeend_entry.insert(END, ":")
        elif len(self.timeend_entry.get()) == 5:
            self.timeend_entry.insert(END, ":")
        elif len(self.timeend_entry.get()) == 9:
    ...
    def add_data(self):
        ...
        # Set default start time and end time if empty
        if self.var_timestart.get() == "":
            self.var_timestart.set("00:00:00")
        if self.var_timeend.get() == "":
            self.var_timeend.set("23:59:59")

        # check_time
        time_start = datetime.strptime(self.var_timestart.get(), '%H:%M:%S').time()
        time_end = datetime.strptime(self.var_timeend.get(), '%H:%M:%S').time()

        if self.var_id.get() == "" or self.dob_entry.get_date().strftime(
                '%d/%m/%Y') == "" or self.var_class.get() == "":
            messagebox.showerror("Error", "Vui lòng nhập đầy đủ thông tin", parent=self.root)
        elif (self.var_class.get() not in arrayClass):
            messagebox.showerror("Error", "Không tồn tại phòng này !!! ", parent=self.root)
            ...
            try:
                ...
                self.reset_data()
                conn.close()
                messagebox.showinfo("Thành công", "Thêm thông tin ca làm việc thành công", parent=self.root)
    ...
    def update_data(self):
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" \
            ...
            messagebox.showerror("Error","Không tồn tại phòng học này !! ",parent=self.root)
        elif (time_end < time_start):
            messagebox.showerror("Error", "Thời gian kết thúc không thể nhỏ hơn thời gian bắt đầu !", parent=self.root)
        else:
            try:
                ...
                if Update>0:
                    ...
                    if not Update:
                        return
                messagebox.showinfo("Thành công","Cập nhật thông tin ca làm việc thành công",parent=self.root)
                conn.commit()
                self.fetch_data()
                self.reset_data()
                conn.close()
    ...
    def search_data(self):
        if self.var_com_search.get()=="" or self.var_search.get()=="":
            ...
            try:
                ...
                if(self.var_com_search.get()=="Phòng"):
                    self.var_com_search.set("Class")
                elif(self.var_com_search.get()=="Mã ca làm việc"):
